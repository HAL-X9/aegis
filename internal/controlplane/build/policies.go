package build

import (
	"fmt"

	"github.com/aegis/internal/controlplane/compiler"
	"github.com/aegis/internal/controlplane/normalize"
	"github.com/aegis/internal/controlplane/schema"
)

// Policies compiles a policy schema into an internal executable representation.
//
// The process is deterministic and consists of two stages:
//  1. Normalization: validates and converts the input schema into a canonical intermediate form.
//  2. Compilation: transforms the normalized representation into a runtime-ready compiled policy model.
//
// The function is side effect free and does not mutate the input schema. It returns an error if either stage fails.
func Policies(schema *schema.Policies) (*compiler.CompiledPolicies, error) {
	normalizedPolicies, err := normalize.Normalize(schema)
	if err != nil {
		return nil, fmt.Errorf("compile policies configuration: normalization failed: %w", err)
	}

	compiledPolicies, err := compiler.CompilePolicy(normalizedPolicies)
	if err != nil {
		return nil, fmt.Errorf("compile policies configuration: policy compilation failed after normalization: %w", err)
	}

	return compiledPolicies, nil
}
