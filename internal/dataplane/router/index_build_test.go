...
func TestBuildRadixTrie(t *testing.T) {
	t.Run("builds empty trie for empty input", func(t *testing.T) {
		...
		if got := trie.Lookup([]byte("/anything")); got != nil {
			t.Fatalf("lookup on empty trie = %#v", got)
		}
	})

	t.Run("indexes all provided routes", func(t *testing.T) {
		r1 := &RouteIndexEntry{Route: &snapshot.CompiledRoute{Name: "r1", Match: snapshot.CompiledMatch{PathPrefix: "/a"}}}
		r2 := &RouteIndexEntry{Route: &snapshot.CompiledRoute{Name: "r2", Match: snapshot.CompiledMatch{PathPrefix: "/b"}}}
		trie := BuildRadixTrie([]*RouteIndexEntry{r1, r2})

		gotA := trie.Lookup([]byte("/a"))
		gotB := trie.Lookup([]byte("/b"))
