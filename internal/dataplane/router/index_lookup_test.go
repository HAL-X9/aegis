...
func TestRadixTrieLookup(t *testing.T) {
	...
	t.Run("prefers exact static route over dynamic", func(t *testing.T) {
		...
		staticEntry := &RouteIndexEntry{
			Route: &snapshot.CompiledRoute{Name: "static"},
		}
		paramEntry := &RouteIndexEntry{
	...
	t.Run("returns nil when no matching edges", func(t *testing.T) {
		...
		trie.Insert(
			"/api/v1",
			&RouteIndexEntry{
				Route: &snapshot.CompiledRoute{Name: "v1"},
			},
		)

		if got := trie.Lookup("/api/v2"); got != nil {
			t.Fatalf("got %#v, want nil", got)
		}
	})
}
