...
func TestRadixTrieLookup(t *testing.T) {
	...
	t.Run("prefers exact static route over dynamic", func(t *testing.T) {
		trie := &RadixTrie{}
		staticEntry := &RouteIndexEntry{Route: &compiler.CompiledRoute{Name: "static"}}
		paramEntry := &RouteIndexEntry{Route: &compiler.CompiledRoute{Name: "param"}}
	...
	t.Run("uses param child when static edge missing", func(t *testing.T) {
		trie := &RadixTrie{}
		entry := &RouteIndexEntry{Route: &compiler.CompiledRoute{Name: "param"}}
	...
	t.Run("uses wildcard child for suffix", func(t *testing.T) {
		trie := &RadixTrie{}
		entry := &RouteIndexEntry{Route: &compiler.CompiledRoute{Name: "wild"}}
	...
	t.Run("returns nil when no matching edges", func(t *testing.T) {
		trie := &RadixTrie{}
		trie.Insert("/api/v1", &RouteIndexEntry{Route: &compiler.CompiledRoute{Name: "v1"}})
